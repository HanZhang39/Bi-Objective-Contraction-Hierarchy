...
public:
    size_t num_obj;
    ZeroH(size_t num_obj): num_obj(num_obj) {};

    std::vector<size_t> get_val(size_t i){
        return std::vector<size_t>(num_obj, 0);
...
public:
    ...
    virtual void operator()(size_t source, std::vector<SolutionSet> &solutions, unsigned int time_limit=UINT_MAX) {}
